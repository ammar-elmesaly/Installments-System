	...
	async getDashboard(options: DashboardQueryOptions = {}): Promise<DashboardOverview> {
		...
		const [dailyAdminRows, receivableRow, archiveRows, archiveTotalCount] = await Promise.all([
			...
			this.transactionsRepository
				...
				.innerJoin('transaction.admin', 'admin')
				.innerJoin('admin.person', 'adminPerson')
				.innerJoin('transaction.installment_plan', 'installmentPlan')
				.innerJoin('installmentPlan.client', 'client')
				.innerJoin('client.person', 'clientPerson')
				.innerJoin('transaction.installment_month', 'installmentMonth')
