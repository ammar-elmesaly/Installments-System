...
export class DashboardService {
  ...
  async getDashboard(
    ...
    const [
      ...
      this.transactionsRepository
        ...
        .where('transaction.created_at BETWEEN :startOfDay AND :endOfDay', {
          startOfDay,
          endOfDay,
        })
        ...
        .addSelect(
          'SUM(CASE WHEN transaction.amount > 0 THEN 1 ELSE 0 END)',
          'collection_count',
        )
        .addSelect(
          'COALESCE(SUM(CASE WHEN transaction.amount > 0 THEN transaction.amount ELSE 0 END), 0)',
          'collected_total',
        )
        .addSelect(
          'COALESCE(SUM(CASE WHEN transaction.amount > 0 AND transaction.payment_type = :cashType THEN transaction.amount ELSE 0 END), 0)',
        ...
        .addSelect(
          'COALESCE(SUM(CASE WHEN transaction.amount < 0 THEN ABS(transaction.amount) ELSE 0 END), 0)',
          'reversal_total',
        )
      ...
      this.installmentMonthsRepository
        ...
        .where('installmentMonth.status = :status', {
          status: InstallmentMonthStatus.Overdue,
        })
        .getRawOne(),
      ...
      this.transactionsRepository
        ...
        .addSelect(
          'installmentMonth.expected_amount',
          'installment_month_expected_amount',
        )
        .addSelect(
          'installmentMonth.paid_amount',
          'installment_month_paid_amount',
        )
    ...
    const accountsReceivable = {
      pending_installments_count: Big(
        receivableRow?.pending_installments_count ?? 0,
      ).toNumber(),
      receivable_amount_total: Big(
        receivableRow?.receivable_amount_total ?? 0,
      ).toNumber(),
    };

    const overdueInstallments = {
      overdue_count: Big(overdueRow?.overdue_count ?? 0).toNumber(),
      overdue_amount_total: Big(
        overdueRow?.overdue_amount_total ?? 0,
      ).toNumber(),
    };
    ...
    return {
      ...
      transaction_log_archive: archiveRows.map((row) =>
        this.mapArchiveRow(row),
      ),
    };
  }
  ...
  private buildCashFlowSummary(rows: any[]): DashboardSummary {
    ...
    return {
      ...
      total_collected: byAdmin.reduce(
        (sum, item) => sum + item.collected_total,
        0,
      ), // Total money collected
      cash_collected_total: byAdmin.reduce(
        (sum, item) => sum + item.cash_collected_total,
        0,
      ), // Hard-cash collected (Not VISA or VODAFONE CASH)
      reversal_count: byAdmin.reduce(
        (sum, item) => sum + item.reversal_count,
        0,
      ),
      reversal_total: byAdmin.reduce(
        (sum, item) => sum + item.reversal_total,
        0,
      ),
      net_total: byAdmin.reduce((sum, item) => sum + item.net_total, 0), // Reversal + Collected
      collection_count: byAdmin.reduce(
