...
describe('ClientsService', () => {
  ...
  it('finds a client by id and throws when it does not exist', async () => {
    ...
    repository.findOne.mockResolvedValueOnce(client).mockResolvedValueOnce(null);

    await expect(service.findById(client.id)).resolves.toBe(client);
    await expect(service.findById(client.id)).rejects.toBeInstanceOf(NotFoundException);
  });
  ...
  it('creates a client in a local transaction and logs the action', async () => {
    ...
    queryRunner.manager.findOne.mockResolvedValueOnce({ person: { admin: {} } }).mockResolvedValueOnce(null);
    queryRunner.manager.create.mockReturnValueOnce(person).mockReturnValueOnce(client);
    queryRunner.manager.save.mockResolvedValueOnce(person).mockResolvedValueOnce(client);
  ...
  it('rejects duplicate phone numbers on client creation', async () => {
    ...
    const dto = { first_name: 'John', last_name: 'Doe', phone_number: '01234567890' } as any;
    queryRunner.isTransactionActive = false;
    queryRunner.manager.findOne.mockResolvedValueOnce({ person: { admin: {} } }).mockResolvedValueOnce({ phone_number: '01234567890' });
    dataSource.createQueryRunner.mockReturnValue(queryRunner);

    await expect(service.create(dto, 'account-id')).rejects.toBeInstanceOf(ConflictException);
  ...
  it('rejects duplicate full names on client creation', async () => {
    const queryRunner = createQueryRunner();
    const dto = { first_name: 'John', second_name: 'M', third_name: 'P', last_name: 'Doe', phone_number: '01234567890' } as any;
    queryRunner.isTransactionActive = false;
    queryRunner.manager.findOne.mockResolvedValueOnce({ person: { admin: {} } }).mockResolvedValueOnce({ first_name: 'John', second_name: 'M', third_name: 'P', last_name: 'Doe' });
    dataSource.createQueryRunner.mockReturnValue(queryRunner);
  ...
  it('throws when admin is not found on creation', async () => {
    ...
    await expect(service.create(dto, 'account-id')).rejects.toBeInstanceOf(NotFoundException);
    expect(queryRunner.rollbackTransaction).toHaveBeenCalled();
  });

  it('updates client and person fields with transaction', async () => {
    const queryRunner = createQueryRunner();
    const client = { id: 'client-id', person: { first_name: 'John', phone_number: '01234567890' }, client_status: ClientStatus.Active } as any;
    const dto = { first_name: 'Jane', client_status: ClientStatus.Inactive } as any;
    queryRunner.manager.findOne.mockResolvedValueOnce({ person: { admin: {} } }).mockResolvedValueOnce(client);
    queryRunner.manager.createQueryBuilder.mockReturnValue({ where: jest.fn().mockReturnThis(), andWhere: jest.fn().mockReturnThis(), getOne: jest.fn().mockResolvedValue(null) });
    queryRunner.manager.merge.mockImplementation((entity, target, values) => Object.assign(target, values));
    queryRunner.manager.save.mockResolvedValue(client);
    dataSource.createQueryRunner.mockReturnValue(queryRunner);

    await service.updateById('client-id', dto, 'account-id');

    expect(queryRunner.manager.merge).toHaveBeenCalledWith(expect.anything(), client.person, { first_name: 'Jane' });
    expect(queryRunner.manager.merge).toHaveBeenCalledWith(expect.anything(), client, { client_status: ClientStatus.Inactive });
    expect(activityLogsService.log).toHaveBeenCalled();
  });
  ...
  it('throws when client is not found on update', async () => {
    ...
    await expect(service.updateById('missing-id', {}, 'account-id')).rejects.toBeInstanceOf(NotFoundException);
    expect(queryRunner.rollbackTransaction).toHaveBeenCalled();
  });

  it('deletes a client and logs the action', async () => {
    const client = { id: 'client-id', person: { id: 'person-id', first_name: 'John', last_name: 'Doe' } } as any;
    ...
    manager.getRepository.mockReturnValueOnce(clientRepo).mockReturnValueOnce(personRepo);
  ...
  it('throws when client is not found on delete', async () => {
    ...
    await expect(service.deleteById('missing-id', 'account-id', manager as any)).rejects.toBeInstanceOf(NotFoundException);
  });
});
